...
Include ICMP types in the dump. Example: C<3>.

=item B<--icmp-code>

Include ICMP codes in the dump. Example: C<8>.

=item B<--icmp-type-name>

Include ICMP types in the dump, using textual names if known. Examples:
C<echo>, C<echo-reply>, C<100>.

=item B<--icmp-code-name>

Include ICMP codes in the dump, using textual names if known. Examples:
