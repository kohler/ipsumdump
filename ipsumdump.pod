...
The `B<-sd>' option tells B<ipsumdump> to log I<s>ource and I<d>estination
IP addresses; `F</home/kohler/largedump.gz>' is a compressed tcpdump(1)
file. Each data line represents a packet; a space separates the two
addresses. The `C<!data>' line describes the contents of each data line.

=head1 OPTIONS

B<Ipsumdump> options fall into three categories. A data source option --
there can be at most one -- says where B<ipsumdump> should read its data.
Dump content options determine which packet characteristics go into the
dump. Finally, other options let you anonymize IP addresses, dump the Click
configuration B<ipsumdump> would run, write packets in parallel to a
tcpdump(1) file, and so on.

=head2 Data Sources

Data source options tell B<ipsumdump> what kind of data to read.
B<Ipsumdump> can read data from tcpdump(1) files (B<--tcpdump>), from live
network interfaces (B<--interface>), from NetFlow summary files
(B<--netflow-summary>), or from B<ipsumdump> output files (B<--ipsumdump>).
...
network interfaces, "eth0" and "eth1". Options that read files --
B<--tcpdump>, B<--netflow-summary>, and B<--ipsumdump> -- read from the
standard input when you supply a single dash C<-> as a filename, and when
you give no filenames at all.
...
not specified by the input files are printed as 0.
...
arbitrary number of them; in the output, fields are separated by spaces. If
you say `B<-sd>', for example -- or the equivalent `B<--src --dst>' -- the
dump's data lines will contain an IP source address, a space, and an IP
destination address:
...
line per record. See also B<--packet-count>, above. If you supply both
B<--multipacket> and B<--packet-count>, then B<--multipacket> is ignored.
...
The `B<-tsSdDp>' option stores the most commonly useful information about
each packet: timestamp, source address, source port, destination address,
destination port, and protocol. Invoking `ipsumdump B<-i> eth1 B<-tsSdDp>'
...
Here is the same data, anonymized with the B<-A> option:
...
B<Ipsumdump> program just builds up a simple Click configuration and runs
it, as if by the click(1) user-level driver. However, you don't need to
install Click to run B<ipsumdump>; the F<libclick> directory contains all
the relevant parts of Click, bundled into a library.

If you're curious, try running `ipsumdump B<--config>' with some other
options to see the Click configuration B<ipsumdump> would run. Only a
handful of source files had to be written from scratch -- particularly,
F<src/ipsumdump.cc>, F<src/toipsumdump.cc>, and F<src/toipsumdump.hh>. The
...
Eddie Kohler <kohler@aciri.org>, based on the Click modular router.
