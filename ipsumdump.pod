...
Include IP fragment offset in the dump. The MF (more fragments) flag is
indicated by a C<+> suffix. Examples: C<0+> (fragment offset 0, more
fragments forthcoming), C<555> (fragment offset 555, this is the last
fragment).
...
Include TCP flags byte in the dump. Example: C<54>. For non-TCP packets,
and for fragments after the first, this field is a single dash C<->.
...
Collate packets from data sources by increasing timestamp. Use this option
when reading from multiple tcpdump(1) files to ensure that the output has
sorted timestamps. When combined with the B<--write-tcpdump> option, this
lets you collate overlapping tcpdump(1) files into a single, sorted
tcpdump(1) file.
...
Output a comment recording the cumulative number of packets output, and the
...
Do not print a progress bar to standard error.
