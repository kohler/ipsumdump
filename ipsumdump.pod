...
B<ipsumdump> output files (B<--ipsumdump>), or files containing
B<tcpdump>'s textual output (B<--tcpdump-text>).
...
Options that read files -- B<--tcpdump>, B<--netflow-summary>,
B<--ipsumdump>, and B<--tcpdump-text> -- read from the standard input when
you supply a single dash C<-> as a filename, or when you give no filenames
at all.
...
someone threw those file away and all you have is the ASCII output, you can
...
Read from one or more DAG-formatted trace files.  Use this option for
new-style ERF dumps (which contain encapsulation type information), or for
old-style dumps with ATM encapsulation.  See
L<http://dag.cs.waikato.ac.nz/>.

=item B<--dag-ppp>

Read from one or more DAG-formatted trace files with old-style PPP
encapsulation.  See L<http://dag.cs.waikato.ac.nz/>.
...
Include UDP length in the dump. This is the length reported in UDP packet
headers; a dash is printed for non-UDP packets, and second and subsequent
UDP fragments. Example: C<1000>.
