...
starting immediately after the newline, consists of binary records.
(`C<!binary>' may be followed by several space characters to ensure that
the first binary record begins on a 4-byte boundary.) 
Each record is a multiple of 4 bytes long, and looks like this:
...
possibly by introducing padding between fields. Some option orders may
introduce unnecessary padding. For example, the records for options
...
newline (possibly padded with zero bytes on the right), same as in a
regular ASCII ipsumdump file. For instance, `C<!count>' records are stored
this way.
