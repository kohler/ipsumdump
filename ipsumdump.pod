...
summary output.  If you include any options that require IP, such as
B<--filter> or a dump content option, then the result will contain only IP
packets (no ARP packets, for example).
