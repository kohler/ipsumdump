...
Finally, other options let you anonymize IP addresses, dump the Click
configuration B<ipsumdump> would run, or write packets in parallel to a
tcpdump(1) file, for example.
...
Read from one or more existing B<ipsumdump> files. Any packet
characteristics not specified by the input B<ipsumdump> files are printed
as 0.
