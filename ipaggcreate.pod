...
Count packets: the output file will report the number of packets in each
aggregate.  This is the default.

=item B<--bytes>, B<-B>

Count bytes: the output file will report the number of bytes in each
aggregate.  This number includes IP and transport headers, but not any link
headers.
...
=item B<--limit-aggregates>=B<count>

Stop after recording information for B<count> aggregates.  That is, exit
just before encountering a packet with the B<count>+1 different aggregate,
or at the end of the trace, whichever comes first.
...
"C<file%03.txt>" will generate files "C<file001.txt>", "C<file002.txt>",
and so forth.
...
=item B<--split-aggregates>=B<count>

Start a new output file every B<count> aggregates.  That is, each file will
contain at most B<count> different aggregates.
...
The initial word of data contains the aggregate number, the second the
count.
